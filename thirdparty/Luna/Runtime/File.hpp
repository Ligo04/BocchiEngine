...
namespace Luna
{
    ...
    {
        none              = 0x00,
        //! This file is a read-only file. One read-only directory denotes a read-only
        //! file system.
        read_only         = 0x01,
    ...
    {
        none           = 0x00,
        //! Grants read access to the file so that @ref IFile::read operations can be
        //! performed.
        read           = 0x01,
        //! Grants write access to the file so that @ref IFile::write operations can
        //! be performed.
        write          = 0x02,
        //! Allocates one user-mode buffer for the opened file.
        //! @details The user-mode buffer can be used to buffer the data read from
        //! file or to be written to file, thus reduce system calls if lots of
        //! small-sized reads/writes need to be performed.
        user_buffering = 0x04,
    };
    ...
    {
        //! Always creates a new file and opens it. If the file already exists, the
        //! old file content will be discarded and the file is treated as a new empty
        //! file.
        create_always        = 1,
        //! Only creates a file and opens it when it does not exist. If the file
        //! already exists, the call fails with @ref BasicError::already_exists.
        create_new           = 2,
        //! Always opens a file. If the file already exists, the file will be open
        //! with its data preserved, if the file does not exist, it will be created
        //! and opened.
        open_always          = 3,
        //! Only opens a file when it does exists. If the file does not exist, the
        //! call fails with @ref BasicError::not_found.
        open_existing        = 4,
        //! Only opens a file when it does exists, and discards the file data so the
        //! file is treated as a new file.
    ...
    struct IFile: virtual ISeekableStream
    {
        ...
        //! Clears all buffers for this stream and causes all buffered data to be
        //! written to the underlying device.
        virtual void flush() = 0;
    };

    //! @interface IFileIterator
    //! Represents a directory stream that can be used to iterate all files and
    //! directories in the specified directory. See @ref open_dir for details.
    struct IFileIterator: virtual Interface
    {
        ...
        //! @return Returns `true` if the iterator points to a valid file, returns
        //! `false` otherwise.
        virtual bool is_valid()                    = 0;

        //! Gets the filename of the file that the iterator currently points to.
        //! @return Returns the name of the file that the iterator currently points
        //! to, returns `nullptr` if the file iterator is invalid. The returnd pointer
        //! is valid until the file iterator is released or @ref move_next is called.
        virtual const c8 *get_filename()           = 0;

        //! Gets the file attribute flags of the file.
        //! @return Returns the file attribute flags of the file that the iterator
        //! currently points to. Returns @ref FileAttributeFlag::none if the file
        //! iterator is invalid.
        ...
        //! If the return value is `false`, then the iterator has reached the end of
    ...
    //! * @ref BasicError::bad_platform_call for all errors that cannot be
    //! identified.
    ...
    {
        none           = 0x00,
        //! If this is specified, the copy operation fails with @ref
        //! BasicError::already_exists if the destination file exists. If this is not
        //! specified, the destination file will be overwritten by the source file in
        //! the copy operation.
        fail_if_exists = 0x01,
    };
    ...
    {
        none           = 0x00,
        //! If this is specified, the move operation fails with @ref
        //! BasicError::already_exists if the destination file exists. If this is not
        //! specified, the destination file will be overwritten by the source file in
        //! the move operation.
        fail_if_exists = 0x01,
    ...
    //! This call can also be used to rename a file.
    //! @param[in] from_path Source file or directory path.
    //! @param[in] to_path Destination file or directory path.
    ...
    //! @param[in] file_path The file or directory to delete. If this is a non-empty
    //! directory, all its contexts will also be deleted.
    //! @par Possible Errors
    //! * @ref BasicError::bad_arguments
    //! * @ref BasicError::not_found
    //! * @ref BasicError::access_denied
    ...
    //! Creates a file iterator that can be used to iterate all files in the
    //! specified directory.
    ...
    //! @details The default current working directory is set to the path that
    //! contains the executable file.
    //! @param[in] buffer_length The length of the buffer for the current directory
    //! string, including the null terminator.
    //! @param[in] buffer A pointer to the buffer that receives the current
    //! directory string. To gets the required buffer size, specify
    //! @ref buffer to `nullptr` and @ref buffer_length to 0.
    //! @return Returns the number of characters copied into the buffer, including
    //! the null terminator. The copied string is always null-terminated. If @ref
    //! buffer_length is `0` and @ref buffer is `nullptr`, returns the required
    //! buffer size to fetch the current directory, including the null terminator.
    LUNA_RUNTIME_API u32 get_current_dir(u32 buffer_length, c8 *buffer);
    //! Sets the current working directory path for the underlying system. The
    //! current directory will be set for the process scope.
    ...
    //! @return Returns the full (absolute) path of the application's executable
    //! file. The returned pointer is valid until Luna SDK is closed.
