...
namespace Bocchi
{
                                        ...
                                        VkMemoryPropertyFlags properties_flag)
    {
        ...
        LOG_ERROR("find MemoryType error");
        return 0;
    }

	VkShaderModule VulkanUtil::createShaderModule(VkDevice device,
                                  ...
                                  VkDeviceMemory&       buffer_memory)
    {
        ...
        vk_buffer_create_info.usage       = usage;                     // use as a vertex/staging/index buffer
        vk_buffer_create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // not sharing among queue families

        if (vkCreateBuffer(device, &vk_buffer_create_info, nullptr, &buffer) != VK_SUCCESS)
        {
            LOG_ERROR("vkCreateBuffer failed!");
            return;
        }

        VkMemoryRequirements vk_buffer_memory_requirements; // for allocate_info.allocationSize and
                                                            // allocate_info.memoryTypeIndex
        vkGetBufferMemoryRequirements(device, buffer, &vk_buffer_memory_requirements);

		VkMemoryAllocateInfo vk_buffer_memory_allocate_info;
        vk_buffer_memory_allocate_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        vk_buffer_memory_allocate_info.allocationSize = vk_buffer_memory_requirements.size;
        vk_buffer_memory_allocate_info.memoryTypeIndex =
            VulkanUtil::findMemoryType(physical_device, vk_buffer_memory_requirements.memoryTypeBits, properties);

        if (vkAllocateMemory(device,&vk_buffer_memory_allocate_info,nullptr,&buffer_memory)!=VK_SUCCESS)
        {
	...
	void VulkanUtil::createBufferAndInitialize(VkDevice              device,
                                               ...
                                               int                   datasize)
    {
        // creaete buffer handle
        VkBufferCreateInfo vk_buffer_create_info {};
        vk_buffer_create_info.sType       = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        vk_buffer_create_info.usage       = usage_flags;
        vk_buffer_create_info.size        = size;
        ...
        vkGetPhysicalDeviceMemoryProperties(physical_device, &vk_physical_device_memory_properties);
        ...
        {
            if ((vk_memory_requirements.memoryTypeBits & 1) == 1)
            {
                if ((vk_physical_device_memory_properties.memoryTypes[i].propertyFlags & memory_property_flags) ==
                ...
                vk_memory_requirements.memoryTypeBits >>= 1;
            }
        }
        ...
        if (vkAllocateMemory(device,&vk_memory_allocate_info,nullptr,memory)!=VK_SUCCESS)
        {
	...
	)
    {
        ...
        VkBufferCopy copy_region = {src_offset, dst_offset, size};
	...
	)
    {
        ...
        vk_image_create_info.flags = image_create_flags;
        vk_image_create_info.imageType = VK_IMAGE_TYPE_2D;
        vk_image_create_info.extent.width = image_width;
        vk_image_create_info.extent.height = image_height;
        vk_image_create_info.mipLevels     = miplevels;
        vk_image_create_info.arrayLayers   = array_layers;
        vk_image_create_info.format        = format;
        vk_image_create_info.tiling        = image_tiling;
        vk_image_create_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        vk_image_create_info.usage         = image_usage_flags;
        vk_image_create_info.samples       = VK_SAMPLE_COUNT_1_BIT;
        vk_image_create_info.sharingMode   = VK_SHARING_MODE_EXCLUSIVE;

        if (vkCreateImage(device,&vk_image_create_info,nullptr,&image)!=VK_SUCCESS)
        {
            LOG_ERROR("failed to craete imamge!");
            return;
        }
        ...
        vk_memory_allocate_info.memoryTypeIndex =
            findMemoryType(physical_device, vk_memory_requirements.memoryTypeBits, memory_property_flags);

        if(vkAllocateMemory(device,&vk_memory_allocate_info,nullptr,&memory)!=VK_SUCCESS)
        {
            LOG_ERROR("failed to allocated image memory!");
            return;
        }
	...
	)
    {
        ...
        switch (texture_image_format)
        {
            ...
            case RHIFormat::RHI_FORMAT_R8G8B8_SRGB:
                texture_byte_size   = texture_image_width * texture_image_height * 3;
                vk_image_format   = VK_FORMAT_R8G8B8_SRGB;
                break;
        ...
        //use the vmAllocate to alloccate asset texture image
        VkImageCreateInfo vk_image_create_info {};
        vk_image_create_info.sType      = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        vk_image_create_info.flags        = 0;
        vk_image_create_info.imageType  = VK_IMAGE_TYPE_2D;
        vk_image_create_info.extent.width = texture_image_width;
        vk_image_create_info.extent.height = texture_image_height;
        vk_image_create_info.extent.depth  = 1;
        vk_image_create_info.mipLevels     = mip_levels;
        vk_image_create_info.arrayLayers   = 1;
        vk_image_create_info.format        = vk_image_format;
    	...
    	vmaCreateImage(static_cast<VulkanRHI*>(rhi)->m_assets_allocator,
                       ...
                       &image_allocation,
                       nullptr);
        //layout transition -- image layout is set from nonre to destination
        ...
        //layout transitions -- image layout is set form destination to shader_read
        transitionImageLayout(rhi,
                              image,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
	...
	void VulkanUtil::createCubeMap(RHI*                        rhi,
                                   ...
                                   uint32_t                    texture_image_width,
                                   uint32_t                    texture_image_height,
                                   const std::array<void*, 6>& texture_image_pixels,
                                   RHIFormat                   texture_image_format,
                                   uint32_t                    miplevels)
    {
        ...
        switch (texture_image_format)
        {
            ...
            case RHIFormat::RHI_FORMAT_R32G32B32A32_SFLOAT:
                ...
                break;
            default:
                texture_layer_byte_size = static_cast<VkDeviceSize>(-1);
        ...
        // use the vmaallocator to allocatore asset texture image
        VkImageCreateInfo vk_image_create_info = {};
        vk_image_create_info.sType             = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        vk_image_create_info.flags             = VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT;
        vk_image_create_info.imageType         = VK_IMAGE_TYPE_2D;
        vk_image_create_info.extent.width      = static_cast<uint32_t>(texture_image_width);
        vk_image_create_info.extent.height     = static_cast<uint32_t>(texture_image_height);
        vk_image_create_info.extent.depth      = 1;
        vk_image_create_info.mipLevels         = miplevels;
        vk_image_create_info.arrayLayers       = 6;
        vk_image_create_info.format            = vulkan_image_format;
        vk_image_create_info.tiling            = VK_IMAGE_TILING_OPTIMAL;
        vk_image_create_info.initialLayout     = VK_IMAGE_LAYOUT_UNDEFINED;
        vk_image_create_info.usage =
            VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        ...
        // layout transitions -- images layout is set from none to destination
        transitionImageLayout(rhi,
                              image,
                              VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        // copy from staging buffer as destination
        copyBufferToImage(rhi, inefficient_staging_buffer, image, texture_image_width, texture_image_height, 6);
                                           ...
                                           VkImageAspectFlags aspect_mask_bits)
    {
        ...
        vk_image_memory_barrier.sType                           = VK_STRUCTURE_TYPE_MEMORY_BARRIER;
        ...
        vk_image_memory_barrier.subresourceRange.aspectMask     = aspect_mask_bits;
        vk_image_memory_barrier.subresourceRange.baseMipLevel   = 0;
        vk_image_memory_barrier.subresourceRange.levelCount     = miplevels;
                 ...
                 new_layout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL)
        {
            ...
            source_stage_flags      = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
            destination_stage_flags = VK_PIPELINE_STAGE_TRANSFER_BIT;
        }
                 ...
                 new_layout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
        {
            ...
            source_stage_flags      = VK_PIPELINE_STAGE_TRANSFER_BIT;
            destination_stage_flags = VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
        }
	...
	void VulkanUtil::genMipmappedImage(RHI* rhi, VkImage image, uint32_t width, uint32_t height, uint32_t mip_levels)
    {
        ...
        for (uint32_t i = 1; i < mip_levels; i++)
        {
            ...
            image_blit.srcOffsets[1].x           = std::max((int32_t)(width >> (i - 1)), 1);
            image_blit.srcOffsets[1].y           = std::max((int32_t)(height >> (i - 1)), 1);
            image_blit.srcOffsets[1].z           = 1;

            image_blit.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
            image_blit.dstSubresource.layerCount = 1;
            image_blit.dstSubresource.mipLevel   = i;
            image_blit.dstOffsets[1].x           = std::max((int32_t)(width >> i), 1);
            image_blit.dstOffsets[1].y           = std::max((int32_t)(height >> i), 1);
	...
	)
    {
        ...
        vk_sampler_create_info.maxAnisotropy    =vk_physical_device_properties.limits.maxSamplerAnisotropy;

        vk_sampler_create_info.borderColor             = VK_BORDER_COLOR_INT_CUSTOM_EXT;
        ...
        vk_sampler_create_info.maxLod = mip_levels - 1;
        if(vkCreateSampler(device,&vk_sampler_create_info,nullptr,&sampler)!=VK_SUCCESS)
        {
	...
	VkSampler VulkanUtil::getOrCreateNearestSampler(VkPhysicalDevice physical_device,
		VkDevice device
	)
    {
	...
	VkSampler VulkanUtil::getOrCreateLinearSampler(VkPhysicalDevice physical_device,
		VkDevice device
	)
    {
