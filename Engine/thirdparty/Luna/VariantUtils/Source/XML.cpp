...
namespace Luna
{
    namespace VariantUtils
    {
        ...
        LUNA_VARIANT_UTILS_API Name get_xml_name(const Variant &xml_element) { return xml_element[_name].str(); }
        LUNA_VARIANT_UTILS_API void set_xml_name(Variant &xml_element, const Name &name) { xml_element[_name] = name; }
        LUNA_VARIANT_UTILS_API const Variant &get_xml_attributes(const Variant &xml_element)
        {
            return xml_element[_attributes];
        }
        LUNA_VARIANT_UTILS_API Variant &get_xml_attributes(Variant &xml_element) { return xml_element[_attributes]; }
        LUNA_VARIANT_UTILS_API const Variant &get_xml_content(const Variant &xml_element)
        {
            return xml_element[_content];
        }
        LUNA_VARIANT_UTILS_API Variant       &get_xml_content(Variant &xml_element) { return xml_element[_content]; }
        LUNA_VARIANT_UTILS_API const Variant &find_first_xml_child_element(const Variant &xml_element,
                                                                           const Name    &name,
                                                                           usize          start_index,
                                                                           usize         *out_index)
        {
        ...
        {
            lucheck(ctx.next_char() == '<' && ctx.next_char(1) == '!' && ctx.next_char(2) == '-'
                    && ctx.next_char(3) == '-');
            ctx.consume('<');
            ctx.consume('!');
            ctx.consume('-');
        ...
        static RV skip_xml_header(IReadContext &ctx)
        {
            ...
            if (!(chs[0] == '<' && chs[1] == '?' && (chs[2] == 'x' || chs[2] == 'X') && (chs[3] == 'm' || chs[3] == 'M')
                  && (chs[4] == 'l' || chs[4] == 'L')))
            {
                return set_error(BasicError::format_error(),
                                 "'<?xml' or '<?XML' expected at the beginning of the document (line %d pos %d).",
                                 ctx.get_line(),
                                 ctx.get_pos());
            }
            ctx.consume(chs[0]);
            ...
            return set_error(BasicError::format_error(),
                             "Unexpected EOF occurred at line %d, pos %d.",
                             ctx.get_line(),
                             ctx.get_pos());
        }
        ...
        {
            return is_name_start_char(ch) || ch == '-' || ch == '.' || (ch >= '0' && ch <= '9') || ch == 0xB7
        ...
        {
            u32 ch_num = (u32)ch;
            return (ch_num >= '0' && ch_num <= '9') || (ch_num >= 'a' && ch_num <= 'f')
                || (ch_num >= 'A' && ch_num <= 'F');
        }
        ...
        static RV read_reference(IReadContext &ctx, String &s)
        {
            ...
            if (ch == '#')
            {
                ...
                if (ch == 'x')
                {
                    ...
                    {
                        return set_error(BasicError::format_error(),
                                         "Unexpected character. (line %d pos %d).",
                                         ctx.get_line(),
                                         ctx.get_pos());
                    }
                    read_ch = atohex(ch);
        ...
        static R<Variant> read_xml_attribute(IReadContext &ctx, Name &attribute_name)
        {
            ...
            lutry
            {
                ...
                if (name.empty())
                    return set_error(BasicError::format_error(),
                                     "Valid name character expected. (line %d pos %d).",
                                     ctx.get_line(),
                                     ctx.get_pos());
                attribute_name = name;
                skip_whitespaces_and_comments(ctx);
        ...
        static RV read_xml_start_tag(IReadContext &ctx, Variant &element, Name &element_name, bool &empty_tag)
        {
            lutry
            {
                ...
                {
                    return set_error(BasicError::format_error(),
                                     "'<' expected at the beginning of one element (line %d pos %d).",
                                     ctx.get_line(),
                                     ctx.get_pos());
                }
                ctx.consume(ch);
        ...
        static R<String> read_xml_cdata(IReadContext &ctx)
        {
            ...
            luassert(ch[0] == '<' && ch[1] == '!' && ch[2] == '[' && ch[3] == 'C' && ch[4] == 'D' && ch[5] == 'A'
                     && ch[6] == 'T' && ch[7] == 'A' && ch[8] == '[');
            ctx.consume(ch[0]);
            ctx.consume(ch[1]);
            ctx.consume(ch[2]);
            ...
            lutry
            {
                ...
                while (true)
                {
                    ...
                    if (ch == '<')
                    {
                        ...
                        else if (ch2 == '!' && ctx.next_char(2) == '[' && ctx.next_char(3) == 'C'
                                 && ctx.next_char(4) == 'D' && ctx.next_char(5) == 'A' && ctx.next_char(6) == 'T'
                                 && ctx.next_char(7) == 'A' && ctx.next_char(8) == '[')
                        {
                            // read CDATA.
        ...
        static RV read_xml_end_tag(IReadContext &ctx, const Name &element_name)
        {
            ...
            if (Name(name) != element_name)
                return set_error(
                    BasicError::format_error(),
                    "The name of the end tag (%s) does not match the name of the start tag (%s). (line %d pos %d)",
                    name.c_str(),
                    element_name.c_str(),
                    ctx.get_line(),
                    ctx.get_pos());
            ...
            if (ch != '>')
                return set_error(BasicError::format_error(),
                                 "'>' expected at the end of one one tag (line %d pos %d).",
                                 ctx.get_line(),
                                 ctx.get_pos());
            ctx.consume(ch);
            return ok;
        ...
        {
            lutry { luexp(skip_xml_header(ctx)); }
            lucatchret;
        ...
        void write_xml_string(String &dst, const Name &str)
        {
            ...
            while (cur < end)
            {
                ...
                switch (ch)
                {
                    ...
                    case '\r': dst.append("&#13;"); break;
                    case '\t': dst.append("&#9;"); break;
                    default: dst.append(cur, utf8_charspan(ch)); break;
                }
